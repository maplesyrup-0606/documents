...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for(unsigned int i=0; i < image.width(); i++){
    for(unsigned int j=0; j < image.height(); j++){
      RGBAPixel* rgbpixel = image.getPixel(i,j);
      int delta_x = (int) i - centerX;
      int delta_y = (int) j - centerY;
      double distance = sqrt(delta_x*delta_x + delta_y*delta_y);
      double dec = 0.005 * distance;

      double dec_r = rgbpixel->r * dec;
      double dec_g = rgbpixel->g * dec;
      double dec_b = rgbpixel->b * dec;

      double final_r = max(0.0 , (double) rgbpixel->r - dec_r);
      double final_g = max(0.0 , (double) rgbpixel->g - dec_g);
      double final_b = max(0.0 , (double) rgbpixel->b - dec_b);

      rgbpixel->r = (unsigned char) final_r;
      rgbpixel->g = (unsigned char) final_g;
      rgbpixel->b = (unsigned char) final_b;
    }
  }
  return image;
}
...
PNG ubcify(PNG image) {
  ...
  for(unsigned int x = 0; x < image.width(); x++){
    for(unsigned int y = 0; y < image.height(); y++){
      RGBAPixel* rgbpixel = image.getPixel(x,y);
      double yellow = colordist(*rgbpixel, ubcyellow);
      double blue = colordist(*rgbpixel, ubcblue);
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for(unsigned int x = 0; x < min(firstImage.width(), secondImage.width()); x++){
    for(unsigned int y = 0; y < min(firstImage.height(), secondImage.height()); y++){
      RGBAPixel* pixel_1 = secondImage.getPixel(x,y);
      if(pixel_1->r == 255 && pixel_1->g == 255 && pixel_1->b == 255){
        RGBAPixel* pixel_2 = firstImage.getPixel(x,y);
        if(pixel_2->r <= 215){
          pixel_2->r += 40;
        }
        else{
